...
const props = defineProps({
  shippingChart: Object,
});

const breadcrumbs = [
  { title: "Dashboard", href: route("dashboard") },
  { title: "Shipping Charts", href: route("admin.shipping-charts.index") },
  { title: "Edit", href: route("admin.shipping-charts.edit", props.shippingChart.id) },
];

const form = useForm({
  _method: "PUT",
  product_type: props.shippingChart.product_type,
  product_condition: props.shippingChart.product_condition,
  product_size_en: props.shippingChart.product_size_en ?? props.shippingChart.product_size ?? "",
  product_size_bn: props.shippingChart.product_size_bn ?? "",
  cutton_contain_amount: props.shippingChart.cutton_contain_amount ?? "",
  shpping_charge_per_cutton: props.shippingChart.shpping_charge_per_cutton ?? "",
});
...
const submit = () => {
  form.post(route("admin.shipping-charts.update", props.shippingChart.id));
};
</script>

<template>
  <Head title="Edit Shipping Chart" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="lg:w-full mx-auto p-4 sm:p-2 lg:p-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-xl font-bold">Edit Shipping Chart</CardTitle>
        </CardHeader>
        <CardContent>
