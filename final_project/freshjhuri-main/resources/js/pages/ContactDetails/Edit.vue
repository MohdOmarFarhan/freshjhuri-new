...
const breadcrumbs = [
    ...
    { title: "Edit", href: `/contact/detail/${props.contactdetail.id}/edit` },
];

const form = useForm({
    image: null,
    phone: props.contactdetail.phone ?? "",
    whats_app: props.contactdetail.whats_app ?? "",
    email: props.contactdetail.email ?? "",
    corporate_office_en: props.contactdetail.corporate_office_en ?? "",
    corporate_office_bn: props.contactdetail.corporate_office_bn ?? "",
    local_office_1_en: props.contactdetail.local_office_1_en ?? "",
    local_office_1_bn: props.contactdetail.local_office_1_bn ?? "",
    local_office_2_en: props.contactdetail.local_office_2_en ?? "",
    local_office_2_bn: props.contactdetail.local_office_2_bn ?? "",
    local_office_3_en: props.contactdetail.local_office_3_en ?? "",
    local_office_3_bn: props.contactdetail.local_office_3_bn ?? "",
});

const existingImageUrl =
    props.contactdetail.image && props.contactdetail.image.startsWith("http")
        ? props.contactdetail.image
        : props.contactdetail.image
        ? `/storage/${props.contactdetail.image}`
        : null;
...
const submit = () => {
    form.transform((data) => ({
        ...data,
        _method: 'PUT',
    })).post(route("contactdetail.update", props.contactdetail.id), {
        forceFormData: true,
    });
};
</script>
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head title="Edit Contact Detail" />

        <div class="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold tracking-tight text-foreground">Edit Contact Detail</h1>
                    <p class="text-muted-foreground">
                        Update how customers can reach your organization across different offices.
                    </p>
                </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <Card>
                    ...
                    <CardContent class="space-y-4">
                        <div class="grid gap-4 md:grid-cols-[1fr,2fr] items-start">
                            ...
                            <div class="space-y-2">
                                <label class="text-sm font-medium text-muted-foreground">Change Image</label>
                                <Input
                                    type="file"
                                    accept="image/*"
                ...
                <div class="flex items-center justify-end gap-3">
                    ...
                    <Button type="submit" :disabled="form.processing">
                        {{ form.processing ? "Updating..." : "Update Contact Detail" }}
                    </Button>
                </div>
            </form>
        </div>
