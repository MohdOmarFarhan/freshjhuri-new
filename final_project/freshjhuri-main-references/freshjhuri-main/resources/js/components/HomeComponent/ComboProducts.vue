...
const filteredProducts = computed(() => {
  if (showAll.value) return props.comboProducts || [];
  return (props.comboProducts || []).slice(0, 8);
});
...
const getComboIncludesLabel = (variant) => {
  const items = variant?.combo_items || variant?.comboItems || [];
  if (!Array.isArray(items) || !items.length) return '';

  const parts = items
    .map((row) => {
      const v = row?.includedVariant || row?.included_variant || null;
      const product = v?.product;
      const title = getLocalizedValue(product, 'title');
      const sizeText = getSizeText(v?.size);
      if (!title) return '';
      return sizeText ? `${title} (${sizeText})` : title;
    })
    .filter(Boolean);
...
const getSeasonLabel = (season) => {
  ...
  switch(seasonLower) {
    ...
    default:
      return t('limited_offer') || 'Limited Offer';
  }
};
...
const getPriceDisplay = (variant) => {
  if (!shouldShowPrice(variant)) {
    return t('price_pending') || 'Price Pending';
  }
...
const addToCart = (variant) => {
  if (variant.product?.season === 'upcoming') {
    alert(t('not_available_yet') || 'This combo will be available soon!');
    return;
  }
...
const sectionTitle = computed(() => {
  return props.title || t('combo_offers');
});
</script>

<template>
  <section class="py-10 lg:py-12 section-bg/20">
    <div class="container mx-auto px-4 lg:px-8">
      
      <div class="flex items-end justify-between mb-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
        <div>
          <span class="text-primary font-semibold text-sm tracking-wider uppercase flex items-center gap-2">
        ...
        <button
          v-if="props.comboProducts?.length > 8 && !showAll"
          @click="showAll = true"
            ...
            <div class="relative w-full h-48 bg-gray-50 dark:bg-zinc-800 rounded-lg overflow-hidden border border-zinc-100 dark:border-zinc-700 flex items-center justify-center">
              ...
              <div class="absolute top-2 left-0 w-full flex flex-row justify-between items-start px-3 z-10 pointer-events-none">
                <div class="flex gap-2">
                  ...
                  <span class="bg-primary text-primary-foreground text-xs font-bold px-2 py-1 rounded shadow" v-if="shouldShowPrice(variant)">
                    🎁 {{ t('combo') }}
                  </span>
                </div>
                <span v-if="variant.product?.season" :class="getSeasonBadgeClass(variant.product.season)" class="text-xs font-semibold px-2 py-1 rounded shadow">
                  {{ getSeasonLabel(variant.product.season) }}
                </span>
              </div>
            </div>
            <div class="flex-1 flex flex-col gap-1 mt-2">
              <span v-if="variant.sku" class="text-[11px] font-semibold uppercase text-primary/80">
                {{ variant.sku }}
              </span>
              ...
              <div v-if="getComboIncludesLabel(variant)" class="text-xs text-primary/80 dark:text-primary/70 font-medium mb-2 bg-primary/5 dark:bg-primary/10 p-2 rounded-lg">
                <!-- <div class="font-semibold mb-1">{{ t('combo_includes') }}:</div> -->
                <div class="text-zinc-600 dark:text-zinc-400 line-clamp-2">
                  {{ getComboIncludesLabel(variant) }}
                </div>
              </div>
              <div class="mt-auto pt-3 border-t border-zinc-100 dark:border-zinc-700 flex flex-row items-center justify-between gap-4">
                ...
                <button
                  ...
                  :disabled="!shouldShowPrice(variant)"
                >
                  <ShoppingCart class="h-5 w-5" />
                  {{ shouldShowPrice(variant) ? t('add_to_cart') : (t('coming_soon') || 'Coming Soon') }}
                </button>
              </div>
            </div>
          </Link>
      ...
      <div v-if="showAll && (props.comboProducts?.length > 8)" class="flex justify-center mt-8">
