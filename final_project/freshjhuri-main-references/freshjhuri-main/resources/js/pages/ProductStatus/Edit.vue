...
const breadcrumbs = [
  ...
  { title: "Edit", href: `/product-statuses/${props.productStatus.id}/edit` },
];

const form = useForm({
  _method: "PUT",
  name_en: props.productStatus.name_en ?? "",
  name_bn: props.productStatus.name_bn ?? "",
});

const submit = () => {
  form.post(route("product-statuses.update", props.productStatus.id), {
    preserveScroll: true,
  });
};
</script>
...
<template>
  <AppLayout :breadcrumbs="breadcrumbs">
    <Head title="Edit Product Status" />
    ...
    <div class="lg:w-full mx-auto p-4 sm:p-2 lg:p-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-xl font-bold">Edit Product Status</CardTitle>
        </CardHeader>
        <CardContent>
