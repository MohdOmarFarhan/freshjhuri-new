...
const breadcrumbs = [
  ...
  { title: 'Edit', href: `/combo-products/${props.comboVariant.id}/edit` },
];
...
const initialItems = computed(() => {
  const items = props.comboVariant?.combo_items || props.comboVariant?.comboItems || [];
  return (items || []).map((row) => ({
    included_variant_id: row.included_variant_id ?? row.includedVariant?.id ?? '',
    qty: row.qty ?? 1,
  }));
});

const initialPackagings = computed(() => {
  const rows = props.comboVariant?.packagings || [];
  if (rows.length) {
    return rows.map((p) => ({
      packaging_system: p.packaging_system ?? defaultPackagingSystem.value,
...
const form = useForm({
  category_id: props.comboVariant?.product?.category_id ?? '',
  title_en: props.comboVariant?.product?.title_en ?? '',
  title_bn: props.comboVariant?.product?.title_bn ?? '',
  short_desc_en: props.comboVariant?.product?.short_desc_en ?? '',
  short_desc_bn: props.comboVariant?.product?.short_desc_bn ?? '',
  conservation_en: props.comboVariant?.product?.conservation_en ?? '',
  conservation_bn: props.comboVariant?.product?.conservation_bn ?? '',
  status: Boolean(props.comboVariant?.product?.status),
  feature_image: null,
  slider_images: [],

  sku: props.comboVariant?.sku ?? '',
  stock: props.comboVariant?.stock ?? 0,
  price: props.comboVariant?.price ?? 0,
  discount_price: props.comboVariant?.discount_price ?? null,

  packagings: initialPackagings.value,
  items: initialItems.value.length ? initialItems.value : [{ included_variant_id: '', qty: 1 }],
  _method: 'PUT',
});

const selectedImagePreviewUrl = ref(null);
const existingImageUrl = computed(() => getImageUrl(props.comboVariant?.product?.feature_image));
const displayImageUrl = computed(() => selectedImagePreviewUrl.value || existingImageUrl.value);
...
const deleteExistingSliderImage = (id) => {
  if (!id) return;
  router.delete(route('products.slider-image.delete', id), { preserveScroll: true });
};
...
const submit = () => {
  form.post(route('combo-products.update', props.comboVariant.id), {
    forceFormData: true,
    preserveScroll: true,
  });
};
...
<template>
  <AppLayout :breadcrumbs="breadcrumbs">
    <Head title="Edit Combo Product" />

    <div class="lg:w-full mx-auto p-4 sm:p-2 lg:p-8 space-y-6">
      <Card>
        <CardHeader class="flex flex-row items-center justify-between">
          <CardTitle class="text-xl font-bold">Combo Package</CardTitle>
          <Button variant="outline" as-child>
            <Link :href="route('combo-products.index')">Back to List</Link>
          </Button>
        </CardHeader>
        <CardContent class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div class="space-y-2 md:col-span-2">
              ...
              <div v-if="displayImageUrl" class="pt-2">
                <img
                  :src="displayImageUrl"
                  alt="Feature"
            ...
            <div class="space-y-2 md:col-span-2">
              ...
              <div v-if="comboVariant.product?.slider_images?.length" class="flex flex-wrap gap-4 mb-4 mt-3">
                <div v-for="img in comboVariant.product.slider_images" :key="img.id" class="relative group">
                  <img
                  ...
                  <button
                    type="button"
                    @click="deleteExistingSliderImage(img.id)"
              ...
              >
                ...
                <div class="md:col-span-1 xl:col-span-2 flex items-center justify-between gap-3">
                  <div class="flex items-center gap-2">
                    <input
                      type="radio"
                      name="default_packaging_combo_edit"
      ...
      <Card>
        <CardHeader class="flex flex-row items-center justify-between">
          <CardTitle class="text-xl font-bold">Combo Items</CardTitle>
        </CardHeader>
        <CardContent class="space-y-4">
          <div class="rounded-lg border border-border bg-muted/20 p-4">
            <p class="font-semibold text-foreground">{{ comboVariant.product?.title_en || '—' }}</p>
            <p class="text-sm text-muted-foreground">Variant ID: {{ comboVariant.id }} | SKU: {{ comboVariant.sku }}</p>
          </div>
          ...
          <form class="space-y-4" @submit.prevent="submit">
            <div v-for="(row, index) in form.items" :key="index" class="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
              <div class="md:col-span-8 space-y-2">
                ...
                <select
                  v-model="row.included_variant_id"
                  class="w-full h-10 rounded-md border border-input bg-background px-3 py-2 text-sm"
                >
              ...
              <div class="md:col-span-2 flex items-center gap-2">
                <Button type="button" variant="outline" @click="addRow">Add</Button>
                <Button type="button" variant="destructive" @click="removeRow(index)">Remove</Button>
              </div>
            </div>

            <div class="flex justify-end pt-2">
              <Button type="submit" :disabled="form.processing">Save Combo Package</Button>
            </div>
          </form>
        </CardContent>
      </Card>
