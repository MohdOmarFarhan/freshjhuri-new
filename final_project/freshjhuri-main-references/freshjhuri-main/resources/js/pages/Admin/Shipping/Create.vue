...
const breadcrumbs = [
    ...
    { title: "Create", href: route('admin.shipping.create') },
];
...
watch(() => form.division_id, async (newDivisionId) => {
    ...
    if (newDivisionId) {
        try {
            const response = await axios.get(route('api.locations.districts', newDivisionId));
...
watch(() => form.district_id, async (newDistrictId) => {
    ...
    if (newDistrictId) {
        try {
            const response = await axios.get(route('api.locations.thanas', newDistrictId));
...
<template>
    <Head title="Create Shipping Cost" />
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="lg:w-full mx-auto p-4 sm:p-2 lg:p-8">
            ...
            <Card>
                <CardHeader>
                    <CardTitle class="text-xl font-bold">Create Shipping Cost</CardTitle>
                </CardHeader>
                <CardContent>
