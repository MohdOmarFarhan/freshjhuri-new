...
const breadcrumbs = [
    ...
    { title: "Edit", href: `/payment-media/${props.paymentmedia.id}/edit` },
];

const form = useForm({
    _method: 'PUT',
    name: props.paymentmedia?.name || '',
    pay_number: props.paymentmedia?.pay_number || '',
    icon: null,
});
...
const imagePreview = ref(getImageUrl(props.paymentmedia?.icon));
...
const submit = () => {
    form.post(route('payment.media.update', props.paymentmedia.id), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head title="Edit Payment Media" />

        <div class="lg:w-full mx-auto p-4 sm:p-2 lg:p-8">
            <Card class="max-w-2xl mx-auto">
                <CardHeader>
                    <CardTitle class="text-xl font-bold">Edit Payment Media</CardTitle>
                </CardHeader>
                <CardContent>
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="flex justify-end gap-4 pt-4">
                            ...
                            <Button type="submit" :disabled="form.processing">
                                {{ form.processing ? 'Updating...' : 'Update Payment Media' }}
                            </Button>
                        </div>
                    </form>
                </CardContent>
