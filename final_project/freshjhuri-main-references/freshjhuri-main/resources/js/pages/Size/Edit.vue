...
const breadcrumbs = [
    ...
    { title: "Edit", href: `/sizes/${props.size.id}/edit` },
];

const form = useForm({
    unit_en: props.size.unit_en,
    unit_bn: props.size.unit_bn,
    amount_en: props.size.amount_en,
    amount_bn: props.size.amount_bn,
    _method: 'PUT',
});

const submit = () => {
    form.post(route('sizes.update', props.size.id), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head :title="`Edit Size: ${size.amount_en} ${size.unit_en}`" />
        ...
        <div class="lg:w-full mx-auto p-4 sm:p-2 lg:p-8">
            ...
            <Card>
                <CardHeader>
                    <CardTitle class="text-xl font-bold">Edit Size</CardTitle>
                </CardHeader>
                <CardContent>
